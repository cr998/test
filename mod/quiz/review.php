...
if(true){
   try{
    $oscarUser=$DB->get_record('user', array('id' => $attemptobj->get_userid()));
    $oscarGrade=quiz_format_grade($quiz, $grade);
    if($oscarGrade>=0){
    $oscarMessage="<html><body>";
    $oscarMessage.="<p>Nota: $oscarGrade</p>";
    $oscarMessage.="<p>Usuario: {$oscarUser->firstname} {$oscarUser->lastname}<p>";
    $oscarMessage.="<p>Email: {$oscarUser->email}</p>";
    $oscarMessage.="<p>Nombre de usuario: {$oscarUser->username}</p>";
    $oscarMessage.="<p>Curso: {$attemptobj->get_course()->fullname}</p>";
    $oscarMessage.="</body></html>";
    $oscarTo = 'diplomas@academiaeninternet.com';
    $oscarSubject = 'Examen aprobado';
    //$oscarHeaders = "From: " . strip_tags($_POST['req-email']) . "\r\n";
    //$oscarHeaders .= "Reply-To: ". strip_tags($_POST['req-email']) . "\r\n";
    //$oscarHeaders .= "CC: susan@example.com\r\n";
    $oscarHeaders .= "MIME-Version: 1.0\r\n";
    $oscarHeaders .= "Content-Type: text/html; charset=ISO-8859-1\r\n";
    mail($oscarTo, $oscarSubject, $oscarMessage, $oscarHeaders);
    //$file = file_get_contents("http://academiaeninternet.com/aulavirtual/diplomaTemplate/diplomaTemplate.php?id={$oscarUser->id}&course={$attemptobj->get_course()->id}&token=549100");
    //$filename="diplomas/diploma{$oscarUser->id}.html";
    /*$f = fopen('php://memory/testdiploma.html', 'w');
    fwrite($f, $file);
    fclose($f);*/
    //file_put_contents($filename, $file);
    $email = new PHPMailer();
    $email->From      = 'oscar@cidegrupo.com';
    $email->FromName  = 'CIDEGRUPO';
    $email->Subject   = 'Diploma';
    $email->Body      = 'Body';
    $email->AddAddress( 'oscar@cidegrupo.com' );
    //$email->AddAddress( $oscarUser->email );

    //$file_to_attach = 'php://memory/testdiploma.html';

    //$email->AddAttachment( $filename , 'Diploma.html' );
    if(file_exists("../../diplomaTemplate/pdfs/pdf{$attemptobj->get_course()->id}.pdf")) $email->AddAttachment("../../diplomaTemplate/pdfs/pdf{$attemptobj->get_course()->id}.pdf", "CaraB.pdf");
    
    //$email->ErrorInfo;
    $email->Send();
    //echo "<script>console.log('Error: {$email->ErrorInfo()}');</script>";
    
    }
    } catch(Exception $e){echo "<script>console.log('Error: {$e->getMessage()}')</script>";}
}
